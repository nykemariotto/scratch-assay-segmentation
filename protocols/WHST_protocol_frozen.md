...
> **Correção datada, 2026-08-04.** Os 86% e os −22,5 pp acima pertencem a conjuntos diferentes e a frase os juntava. Os −22,5 pp (mediana) são do subconjunto de **96** quadros em que existe contorno automático E corrigido (`data/correction_agreement.csv`), e nesse conjunto a redução ocorre em **85%** (82/96), não em 86% — o 86 não é reproduzível por nenhum recorte do dado. Sobre os **132 dos 140** quadros corrigidos com as duas áreas registradas, a mediana é **−11,3 pp** e **71%** são reduções, porque as passadas de completação, baseline e validação incluem quadros em que o plugin não devolveu contorno e a correção só podia acrescentar área. O dado não mudou: o CSV regenera idêntico.
