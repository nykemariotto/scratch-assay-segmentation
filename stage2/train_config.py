...
def main():
    ...
    # padding ANTES de importar/instanciar o pipeline de dados
    ...
    # ------------------------------------------------------------------
    # EVIDENCIA DE PADDING GRAVADA DE DENTRO DO TREINO REAL.
    #
    # Em 2026-07-27 perdemos 34 h porque o patch de padding valia no processo pai
    ...
    # lugar certo no processo errado.
    #
    # This callback reads the FIRST BATCH TRAINING ACTUALLY CONSUMES and records the
    # histogram of the extreme values. If the padding does not arrive, it is recorded in
    # proprio run, e o preflight/canario detecta antes de gastar as 34 h de novo.
    # ------------------------------------------------------------------
    ...
    # registro de proveniencia NO DIRETORIO REAL do Ultralytics (save_dir),
    ...
    if evidencia and "erro" not in evidencia:
        ...
        if fill != 114 and f114 > 0.5 * falvo:
            aviso = (f"SUSPEITO: cinza(114) em {100*f114:.2f}% dos pixels contra "
                     f"{100*falvo:.2f}% of the requested padding ({fill}). The patch may not "
                     f"ter chegado aos workers — ver D12.")
        elif falvo < 0.005:
            aviso = (f"SUSPEITO: padding pedido ({fill}) aparece em apenas "
    ...
    # ---- sentinela de conclusao (ESCRITO POR ULTIMO, so apos exit 0) ----
    # Conta as epocas efetivamente concluidas a partir do results.csv e marca se
