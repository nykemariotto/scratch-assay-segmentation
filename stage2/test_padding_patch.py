# -*- coding: utf-8 -*-
"""
stage2/test_padding_patch.py — gate do patch de padding. Roda ANTES de qualquer treino.

It exists because of the 2026-07-27 defect: the patch held in the parent process and not
nos workers, e a grade inteira treinou com padding CINZA nos dois bracos da
ablation. Nada no log denunciou — os treinos correram 34 h e produziram numeros
plausible. Only a bit-by-bit comparison of the weights revealed it.

Este teste falha se isso voltar a acontecer.
"""
import os
import sys
...
def check(cond, rot, extra=""):
    print(f"  [{'ok ' if cond else 'FALHA'}] {rot}{('  ' + extra) if extra else ''}")
    if not cond:
        falhas.append(rot)
...
class Sonda(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return {"pid": os.getpid(),
                "modo": (pp.applied() if pp else None) or "NENHUM",
...
def main():
    ...
    print("1. patch no processo pai")
    for modo, esperado in (("black", 0), ("white", 255)):
        fill = padding_patch.apply(modo)
        check(fill == esperado, f"apply({modo!r}) -> {fill}", f"(esperado {esperado})")
        check(int(np.full((1,), 114, dtype=np.uint8)[0]) == esperado,
              f"np.full(114) no pai devolve {int(np.full((1,), 114, dtype=np.uint8)[0])}")
    ...
    check(B.seed_worker is padding_patch.worker_init,
          "build.seed_worker substituido por padding_patch.worker_init")
    ...
    for modo, esperado in (("white", 255), ("black", 0)):
        ...
        check(len(workers) >= 1, f"[{modo}] workers observados: {len(workers)}")
        for p, r in workers.items():
            check(r["modo"] == modo and r["full114"] == esperado,
                  f"[{modo}] worker {p}: modo={r['modo']} np.full(114)->{r['full114']}",
                  f"(esperado {modo}/{esperado})")
    ...
    check(not np.array_equal(a, b), "o mesmo lote difere entre black e white")
    ...
    check(dif < 0.05,
          ...
          f"(black {n114_a}, white {n114_b}, diferença {100*dif:.1f}%)")
