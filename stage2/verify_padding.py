...
# ---------- caminho de TREINO (augment=True -> Mosaic + RandomPerspective) ----------
...
print(f"TREINO  shape={arr.shape} dtype={arr.dtype} min={arr.min()} max={arr.max()}")
vals, cnts = np.unique(arr, return_counts=True)
top = sorted(zip(cnts, vals), reverse=True)[:5]
print(f"  valores mais frequentes: {[(int(v), int(c)) for c, v in top]}")

# ---------- caminho de VAL (augment=False -> LetterBox) ----------
...
# as barras de padding ficam no topo/base (imagens 2452x2056 -> AR 1.19)
top_rows = a2[:12].reshape(-1, a2.shape[-1])
bot_rows = a2[-12:].reshape(-1, a2.shape[-1])
print(f"  media das 12 linhas do TOPO : {top_rows.mean(axis=0).round(1)}")
print(f"  media das 12 linhas da BASE : {bot_rows.mean(axis=0).round(1)}")
uniq_t = np.unique(top_rows)
print(f"  valores unicos no topo (max 8): {uniq_t[:8]}")

esperado = val
ok_val = abs(float(top_rows.mean()) - esperado) < 3
print(f"\n  esperado={esperado}  -> padding do VAL {'OK' if ok_val else 'DIVERGENTE'}")
