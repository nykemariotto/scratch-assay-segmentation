...
print(f"[1] RAW: {len(raw)} arquivos, {len(byh)} hashes distintos, {len(dups)} hashes duplicados")
dup_report = []
for h, v in dups.items():
    lotes = sorted({x["batch"] for x in v})
    print(f"  hash {h[:12]}  n={len(v)}  lotes={lotes}")
    for x in v:
...
only_raw = raw_h - bd_h
only_bd = bd_h - raw_h
print(f"\n[4] BD: {len(bd)} arquivos, {len(bd_h)} hashes distintos")
...
print(f"    BD hashes duplicados: {len(bd_dups)}")
...
print(f"\n[2/3] linhas HUVEC no mapping: {len(hv)}")
semfonte = [r for r in hv if not r["lote"].strip()]
print(f"      sem_fonte (lote vazio): {len(semfonte)}")
...
print(f"      well recuperavel por nome: {n_well}/{len(detalhe)}; timepoint: {n_tp}/{len(detalhe)}")
for d in detalhe[:15]:
...
print(f"    com (lote,trat) por hash            : {com_lote}")
print(f"    + recuperadas por nome normalizado  : {res['recuperado_unico']}")
print(f"    = com (lote,trat)                   : {final_ok}")
print(f"    ambiguas entre lotes                : {res['ambiguo_multi_lote']}")
print(f"    with no source at all               : {res['irrecuperavel']}")
print(f"    ctx_ambiguo no join por hash        : {amb}")
print(f"    well_campo preenchido               : {well_ok}/{len(hv)}")
# completo = lote + trat + well
completo = 0
for r in hv:
    if r["lote"].strip() and r["well_campo"].strip():
        completo += 1
print(f"    (lote & well) completo por hash     : {completo}")
comp_final = completo + sum(1 for d in detalhe if d["status"] == "unico" and d["well"])
print(f"    (lote & well) completo apos recup.  : {comp_final}")
OUT["hv_total"] = len(hv)
