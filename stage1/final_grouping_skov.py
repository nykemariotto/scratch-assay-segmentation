...
def viability(ng, label):
    ...
    print(f"  [{label:<28}] {ng:>4} grupos -> train={ntr} val={nv} test={nt}  {verd}")


print("=== SKOV: contagem por chave candidata ===")
...
print(f"\nImagens por grupo de campo: min={szv[0]} mediana={szv[len(szv)//2]} max={szv[-1]}")
print(f"Grupos de campo totais: {len(gsz)}")
...
for k in sorted(cov):
    ...
    print(f"  {k[0]}/{k[1]:<10}: {len(fields)} campos, ids {fields[:16]}")
