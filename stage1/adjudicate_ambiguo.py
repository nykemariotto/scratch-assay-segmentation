# -*- coding: utf-8 -*-
"""
stage1/adjudicate_ambiguo.py — adjudication of the 12 AMBIGUO cases before the correction.

Fluxo:
  1) python stage1/adjudicate_ambiguo.py --template
       writes stage1/adjudicacao_ambiguo.csv with one row per AMBIGUO and the
       'decisao' column EMPTY, to be filled in while looking at inspect_ambiguo.png.
       Valores aceitos: OK | super | sub | invalida
  2) (voce preenche a coluna decisao)
  3) python stage1/adjudicate_ambiguo.py --apply
       valida o preenchimento, aplica em data/inspecao_visual.csv (categoria/subtipo),
...
ADJ = "stage1/adjudicacao_ambiguo.csv"
...
def template():
    ...
    if not amb:
        sys.exit("nenhum AMBIGUO em data/inspecao_visual.csv (ja adjudicado?)")
    rows = []
    ...
    print("Use o painel inspect_ambiguo.png (vermelho = o frame a adjudicar).")
    print("Depois rode: python stage1/adjudicate_ambiguo.py --apply")


def apply():
    if not os.path.isfile(ADJ):
        sys.exit(f"{ADJ} does not exist. Run first: python stage1/adjudicate_ambiguo.py --template")
