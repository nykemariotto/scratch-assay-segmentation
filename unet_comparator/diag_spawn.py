...
def mede(ds, workers, persistent, n_ciclos=3, n_lotes=2):
    """tempo ate o PRIMEIRO lote sair — e o custo de subir os workers."""
    g = torch.Generator()
    ...
    for _ in range(n_ciclos):
        t = time.perf_counter()
        it = iter(dl)                      # <- aqui o spawn acontece
        next(it)                           # espera o primeiro lote
