...
if not alvos:
    ...
    print(f"     epochs {d['completed_epochs']} · non-finite {d['nan_epochs']}"
          f" (1a na {d['primeira_epoca_nao_finita']}) · degeneradas"
    ...
    ok(d["primeira_epoca_nao_finita"] == 41,
       f"aponta a epoca 41 como primeira (viu {d['primeira_epoca_nao_finita']})")
...
# ── 4. tres seguidas DEVEM reprovar ──────────────────────────────────────────
...
d = diagnostica_csv(p, 100)
ok(reprovado(d, 100),
   f"tres consecutivas reprovam (degeneradas={d['epocas_iou_igual_dice']})")
os.unlink(p)
