...
# -*- coding: utf-8 -*-
"""
stage3/tres_vias.py — classical × YOLO × U-Net, on the same outcome and the same set.

`stage3/benchmark_classico_longo.csv` carries the closure fraction of the AUTOMATIC
...
# ── classical ───────────────────────────────────────────────────────────────
cls = {}
for r in csv.DictReader(open("stage3/benchmark_classico_longo.csv", encoding="utf-8-sig")):
...
* ⚠️ THE CLASSICAL ARM SUFFERS FROM INCORPORATION BIAS. The reference standard was
  ...
  This is STARD's incorporation bias, which is why `benchmark_classico.py` already
  declares it.""")
