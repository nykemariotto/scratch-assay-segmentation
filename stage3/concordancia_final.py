...
# -*- coding: utf-8 -*-
"""
stage3/concordancia_final.py — every agreement number the manuscript states.
...
`stage3/estatisticas_para_224.py` already gave (Pearson, Spearman, CCC, bias, LoA,
...
ONE DEFINITION ONLY. `ccc` and `tost` come from `stage3/_concordancia_estat.py`,
the same module `stage3/estatisticas_para_224.py` imports. Two implementations of
...
from _concordancia_estat import ccc, tost               # noqa: E402
...
MARGENS = [0.10, 0.05]
DESTINO = os.path.join("stage3", "concordancia_final.json")
DEST_CSV = os.path.join("stage3", "concordancia_final_longo.csv")

# Values the manuscript's Discussion ALREADY reports, from
# `stage3/estatisticas_para_224.py`. This script reconstructs the selection of
...
def carrega():
    ...
    with open("data/closure_final_longo.csv", encoding="utf-8-sig") as fh:
...
def main():
    ...
    if sem_linhagem:
        sys.exit(f"{len(sem_linhagem)} observation(s) with no cell line in "
                 f"data/closure_final_longo.csv — the join is broken, e.g.: {sem_linhagem[:2]}")
    ...
    out = {
        "fonte": "stage3/cmp_*_paired_new_longo.csv (stage3/paired_new.py)",
